    ...
    pub fn formatAsMarkdown(self: *const ParetoFrontier, allocator: std.mem.Allocator) ![]u8 {
        ...
        for (self.points) |point| {
            if (!point.is_dominated) {
                try result.appendSlice(allocator, try point.formatAsMarkdown(allocator));
            }
        }
        ...
        try result.appendSlice(allocator, "\n**Coverage**: ");
        try result.appendSlice(allocator, try std.fmt.allocPrint(allocator, "{d:.1}% ({d}/{d} models are Pareto optimal)\n", .{ coverage, optimal_count, total_models }));
...
pub const GitHubActionsBadge = struct {
    ...
    pub fn generateBadgesMarkdown(self: *const GitHubActionsBadge, allocator: std.mem.Allocator, build_status: ?[]const u8, coverage_pct: ?f64) ![]u8 {
        ...
        if (build_status) |status| {
            ...
            try result.appendSlice(allocator, try std.fmt.allocPrint(allocator, "![Build Status]({s})\n\n", .{badge_url}));
        }
        ...
        if (coverage_pct) |coverage| {
            ...
            try result.appendSlice(allocator, try std.fmt.allocPrint(allocator, "![Coverage]({s})\n\n", .{badge_url}));
        }
