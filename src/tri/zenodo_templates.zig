...
pub const SignificanceLevel = enum {
    ...
    /// V14: Emoji representation for markdown
...
pub const BibTexEntry = struct { cite_key: []const u8, entry_type: BibTexEntryType, title: []const u8, author: []const u8, journal: ?[]const u8 = null, booktitle: ?[]const u8 = null, publisher: ?[]const u8 = null, year: u32, volume: ?u32 = null, number: ?u32 = null, pages: ?[]const u8 = null, doi: ?[]const u8 = null, url: ?[]const u8 = null, note: ?[]const u8 = null, series: ?[]const u8 = null, editor: ?[]const u8 = null, chapter: ?[]const u8 = null, address: ?[]const u8 = null, edition: ?[]const u8 = null, howpublished: ?[]const u8 = null };
...
// ============================================================================
// V14: Experiment Comparison — Cross-Experiment Analysis
// ============================================================================

// Note: SignificanceLevel enum already exists in the file (line 2864)
// Adding toEmoji extension method here via comptime extension would require
// modifying the original. For now, using symbols directly in ExperimentComparison.
...
/// Cross-experiment comparison with statistical testing
pub const ExperimentComparison = struct {
    caption: []const u8,
    label: []const u8,
    comparison_metric: []const u8,
    higher_is_better: bool,
    results: []const ExperimentResult,
    statistical_test: ?[]const u8 = null,
    significance_level: f64 = 0.05,
    ci_percent: f64 = 0.95,

    pub fn formatAsLaTeX(self: *const ExperimentComparison, allocator: std.mem.Allocator) ![]u8 {
        var buffer = try std.ArrayList(u8).initCapacity(allocator, 0);
        defer buffer.deinit();
        ...
        try buffer.appendSlice("}\n");
        try buffer.appendSlice("\\label{");
        try buffer.appendSlice(self.label);
        try buffer.appendSlice("}\n");
        try buffer.appendSlice("\\begin{tabular}{l");

        // Build column spec based on data
        try buffer.appendSlice("cc");
        if (self.statistical_test != null) try buffer.appendSlice("c");
        if (self.results[0].std_err != null) try buffer.appendSlice("c");
        if (self.results[0].ci_lower != null) try buffer.appendSlice("cc");
        try buffer.appendSlice("}\n");
        ...
        try buffer.writer().print("{:.2}", .{self.comparison_metric});
        try buffer.appendSlice(if (self.higher_is_better) " \\uparrow)" else " \\downarrow)");
        try buffer.appendSlice(" & ");

        if (self.results[0].std_err != null) try buffer.appendSlice("SE & ");
        if (self.results[0].ci_lower != null) {
            try buffer.appendSlice("95\\% CI & ");
        }
        if (self.statistical_test != null) try buffer.appendSlice("Signif. \\\\\n");
        try buffer.appendSlice("\\midrule\n");

        // Data rows
        for (self.results) |result| {
            try buffer.appendSlice(result.experiment_name);
            if (result.is_best) {
                try buffer.appendSlice(" \\textbf{");
            }
            try buffer.writer().print(" & {d}", .{@intFromFloat(result.metric_value * 100)});
            if (result.is_best) {
                try buffer.appendSlice("}");
            }
            try buffer.appendSlice(" & ");
            ...
            if (result.std_err) |se| {
                try buffer.writer().print("{:.2f}", .{se});
            }
            try buffer.appendSlice(" & ");

            if (result.ci_lower) |lower| {
                try buffer.writer().print("[{:.2f},", .{lower});
            }
            if (result.ci_upper) |upper| {
                try buffer.writer().print("{:.2f}]", .{upper});
            }
            try buffer.appendSlice(" & ");

            if (self.statistical_test != null) {
                const sig_level = self.significanceLevelFor(result);
                try buffer.appendSlice(sig_level.toSymbol());
            }
            try buffer.appendSlice(" \\\\\n");
        }
        ...
        if (self.statistical_test) |stat_test| {
            try buffer.appendSlice("\\vspace{0.2em}\n");
            try buffer.appendSlice("{\\small ");
            try buffer.appendSlice("Statistical test: ");
            try buffer.appendSlice(stat_test);
            try buffer.appendSlice(", $\\alpha = ");
            try buffer.writer().print("{:.2f}$", .{self.significance_level});
            try buffer.appendSlice(".}\n");
        }
    ...
    pub fn formatAsMarkdown(self: *const ExperimentComparison, allocator: std.mem.Allocator) ![]u8 {
        var buffer = try std.ArrayList(u8).initCapacity(allocator, 0);
        defer buffer.deinit();
        ...
        try buffer.writer().print("{:.2}", .{self.comparison_metric});
        try buffer.appendSlice(if (self.higher_is_better) " ↑)" else " ↓)");

        if (self.results[0].std_err != null) try buffer.appendSlice(" | SE");
        if (self.results[0].ci_lower != null) try buffer.appendSlice(" | 95% CI");
        if (self.statistical_test != null) try buffer.appendSlice(" | Signif.");
        try buffer.appendSlice(" |\n");
        try buffer.appendSlice("|--------|");
        if (self.higher_is_better) {
        ...
        for (self.results) |result| {
            try buffer.appendSlice("| ");
            if (result.is_best) try buffer.appendSlice("**");
            try buffer.appendSlice(result.experiment_name);
            if (result.is_best) try buffer.appendSlice("**");
            try buffer.appendSlice(" | ");

            try buffer.writer().print("{:.2}", .{result.metric_value});
            try buffer.appendSlice(" | ");
            ...
            if (self.statistical_test) |_| {
                const sig_level = self.significanceLevelFor(result);
                try buffer.appendSlice(sig_level.toEmoji());
            }
            try buffer.appendSlice(" |\n");
        }
        ...
        if (self.statistical_test) |stat_test| {
            try buffer.appendSlice("*Statistical test: ");
            try buffer.appendSlice(stat_test);
            try buffer.writer().print(", $\\alpha = {:.2f}$\n", .{self.significance_level});
        }
...
test "ExperimentComparison formatAsLaTeX" {
    const results = [_]ExperimentResult{
        .{
    ...
    const comp = ExperimentComparison{
    ...
    const latex = try comp.formatAsLaTeX(std.testing.allocator);
    defer std.testing.allocator.free(latex);

    try std.testing.expect(std.mem.indexOf(u8, latex, "\\begin{table}") != null);
    try std.testing.expect(std.mem.indexOf(u8, latex, "HSLM-1.95M (ours)") != null);
    try std.testing.expect(std.mem.indexOf(u8, latex, "\\textbf{125}") != null);
    try std.testing.expect(std.mem.indexOf(u8, latex, "[121.5, 129.9]") != null);
}

test "ExperimentComparison formatAsMarkdown" {
    const results = [_]ExperimentResult{
        .{
            .experiment_name = "HSLM-1.95M",
    ...
    try std.testing.expect(std.mem.indexOf(u8, md, "# Comparison") != null);
    try std.testing.expect(std.mem.indexOf(u8, md, "**HSLM-1.95M**") != null);
    try std.testing.expect(std.mem.indexOf(u8, md, "125.7") != null);
}

// ============================================================================
// V14: Conference Metadata — Conference-Specific Requirements
// ============================================================================

/// Presentation type at conference
...
/// Conference metadata with presentation info
pub const ConferenceMetadata = struct {
    ...
    primary_contact: ?[]const u8 = null,
    corresponding_orcid: ?[]const u8 = null,

    pub fn formatAsLaTeX(self: *const ConferenceMetadata, allocator: std.mem.Allocator) ![]u8 {
        var buffer = try std.ArrayList(u8).initCapacity(allocator, 0);
        defer buffer.deinit();

        try buffer.appendSlice("\\section*{Conference Information}\n\n");

        try buffer.appendSlice("\\textbf{Conference}: ");
        try buffer.appendSlice(self.conference.name);
        try buffer.appendSlice(" (");
        try buffer.appendSlice(self.conference.acronym);
        try buffer.writer().print(" {d})", .{self.conference.year});
        try buffer.appendSlice("\n\n");

        if (self.conference.location) |loc| {
            try buffer.appendSlice("\\textbf{Location}: ");
            try buffer.appendSlice(loc);
            try buffer.appendSlice("\n\n");
        }

        if (self.conference.dates) |dates| {
            try buffer.appendSlice("\\textbf{Dates}: ");
            try buffer.appendSlice(dates);
            try buffer.appendSlice("\n\n");
        }

        try buffer.appendSlice("\\textbf{Presentation}: ");
        try buffer.appendSlice(self.presentation.presentation_type.toString());
        if (self.presentation.track) |track| {
            try buffer.appendSlice(", Track: ");
            try buffer.appendSlice(track);
        }
        if (self.presentation.session) |session| {
            try buffer.appendSlice(", Session: ");
            try buffer.appendSlice(session);
        }
        try buffer.appendSlice("\n\n");

        if (self.conference.website) |website| {
            try buffer.appendSlice("\\textbf{Website}: \\url{");
            try buffer.appendSlice(website);
            try buffer.appendSlice("}\n\n");
        }

        if (self.corresponding_orcid) |orcid| {
            try buffer.appendSlice("\\textbf{Corresponding Author ORCID}: \\href{https://orcid.org/");
    ...
    pub fn formatAsMarkdown(self: *const ConferenceMetadata, allocator: std.mem.Allocator) ![]u8 {
        var buffer = try std.ArrayList(u8).initCapacity(allocator, 0);
        defer buffer.deinit();

        try buffer.appendSlice("# Conference Information\n\n");

        try buffer.appendSlice("## Conference\n\n");
        try buffer.appendSlice("- **Name**: ");
        try buffer.appendSlice(self.conference.name);
        try buffer.appendSlice(" (");
        try buffer.appendSlice(self.conference.acronym);
        try buffer.writer().print(" {d})\n", .{self.conference.year});

        if (self.conference.location) |loc| {
            try buffer.appendSlice("- **Location**: ");
            try buffer.appendSlice(loc);
            try buffer.appendSlice("\n");
        }
        ...
        if (self.presentation.room) |room| {
            try buffer.appendSlice("\n- **Room**: ");
            try buffer.appendSlice(room);
        }

        if (self.presentation.time) |time| {
            try buffer.appendSlice("\n- **Time**: ");
            try buffer.appendSlice(time);
        }
        ...
        if (self.keywords) |kw| {
            ...
            for (kw) |keyword| {
                try buffer.appendSlice("- ");
                try buffer.appendSlice(keyword);
                try buffer.appendSlice("\n");
            }
        }
...
test "ConferenceMetadata formatAsLaTeX" {
    ...
    const pres = PresentationDetails{
    ...
    const meta = ConferenceMetadata{
        .title = "HSLM: Ternary Language Models",
        .conference = conf,
        .presentation = pres,
    };

    const latex = try meta.formatAsLaTeX(std.testing.allocator);
    defer std.testing.allocator.free(latex);

    try std.testing.expect(std.mem.indexOf(u8, latex, "\\section*{Conference Information}") != null);
    try std.testing.expect(std.mem.indexOf(u8, latex, "ICLR") != null);
    try std.testing.expect(std.mem.indexOf(u8, latex, "Oral") != null);
}

test "ConferenceMetadata formatAsMarkdown" {
    const conf = ConferenceInfo{
    ...
    const md = try meta.formatAsMarkdown(std.testing.allocator);
    defer std.testing.allocator.free(md);

    try std.testing.expect(std.mem.indexOf(u8, md, "# Conference Information") != null);
    try std.testing.expect(std.mem.indexOf(u8, md, "NeurIPS") != null);
    try std.testing.expect(std.mem.indexOf(u8, md, "Poster") != null);
}

// ============================================================================
// V14: Author List — Detailed Author Information with Affiliations
// ============================================================================

/// Institutional affiliation
pub const Affiliation = struct {
...
/// Detailed author with affiliations and ORCID
pub const DetailedAuthor = struct {
    first_name: []const u8,
    last_name: []const u8,
    middle_initial: ?u8 = null,
    orcid: ?[]const u8 = null,
    affiliations: []const Affiliation,
    is_corresponding: bool = false,
    is_equal_contribution: bool = false,

    pub fn fullName(self: *const DetailedAuthor, allocator: std.mem.Allocator) ![]u8 {
        var buffer = try std.ArrayList(u8).initCapacity(allocator, 0);
        defer buffer.deinit();

        try buffer.appendSlice(self.first_name);
        try buffer.appendSlice(" ");

        if (self.middle_initial) |mi| {
            try buffer.appendSlice(&.{mi});
            try buffer.appendSlice(". ");
        }

        try buffer.appendSlice(self.last_name);

        return buffer.toOwnedSlice();
    }

    pub fn toLaTeX(self: *const DetailedAuthor, allocator: std.mem.Allocator) ![]u8 {
        var buffer = try std.ArrayList(u8).initCapacity(allocator, 0);
        defer buffer.deinit();

        const name = try self.fullName(allocator);
        defer allocator.free(name);

        try buffer.appendSlice(name);

        if (self.is_equal_contribution) {
            try buffer.appendSlice("$^{\\dagger}$");
        }
...
/// Author list with multiple affiliations
pub const AuthorList = struct {
    title: []const u8,
    authors: []const DetailedAuthor,
    corresponding_email: ?[]const u8 = null,
    equal_contribution_note: ?[]const u8 = null,

    pub fn formatAsLaTeX(self: *const AuthorList, allocator: std.mem.Allocator) ![]u8 {
        var buffer = try std.ArrayList(u8).initCapacity(allocator, 0);
        defer buffer.deinit();
        ...
        for (self.authors, 0..) |author, i| {
            const name = try author.toLaTeX(allocator);
            defer allocator.free(name);
            try buffer.appendSlice(name);
        ...
        // Affiliations
        try buffer.appendSlice("\\affiliation{");

        var affiliation_set = std.StringHashMap(void).init(allocator);
        defer affiliation_set.deinit();

        for (self.authors) |author| {
            for (author.affiliations) |aff| {
                const key = try std.fmt.allocPrint(allocator, "{s}||{s}||{s}", .{
                    aff.institution, aff.department orelse "", aff.city orelse "",
                });
                defer allocator.free(key);

                if (affiliation_set.fetchPut(key, {}) == null) {
                    try buffer.appendSlice(aff.institution);
                    if (aff.department) |dept| {
                        try buffer.appendSlice(", ");
                        try buffer.appendSlice(dept);
                    }
                    if (aff.city) |city| {
                        try buffer.appendSlice(", ");
                        try buffer.appendSlice(city);
                    }
                    try buffer.appendSlice("\\\\");
                }
            }
        ...
        if (self.isCorrespondingAuthor()) {
            try buffer.appendSlice("\\thanks{*Corresponding author");
            if (self.corresponding_email) |email| {
                try buffer.appendSlice(": \\texttt{");
        ...
        if (self.hasEqualContribution()) {
            try buffer.appendSlice("\\thanks{$^{\\dagger}$Equal contribution");
            if (self.equal_contribution_note) |note| {
                try buffer.appendSlice(" (");
                try buffer.appendSlice(note);
                try buffer.appendSlice(")");
            }
    ...
    fn isCorrespondingAuthor(self: *const AuthorList) bool {
        for (self.authors) |author| {
            if (author.is_corresponding) return true;
        }
        return false;
    }

    fn hasEqualContribution(self: *const AuthorList) bool {
        for (self.authors) |author| {
            if (author.is_equal_contribution) return true;
        }
        return false;
    }

    pub fn formatAsMarkdown(self: *const AuthorList, allocator: std.mem.Allocator) ![]u8 {
        var buffer = try std.ArrayList(u8).initCapacity(allocator, 0);
        defer buffer.deinit();
        ...
        for (self.authors) |author| {
            const name = try author.fullName(allocator);
            defer allocator.free(name);

            try buffer.appendSlice("### ");
            try buffer.appendSlice(name);
            if (author.is_corresponding) try buffer.appendSlice(" (Corresponding)");
            if (author.is_equal_contribution) try buffer.appendSlice(" †");
            try buffer.appendSlice("\n\n");

            if (author.orcid) |orcid| {
                try buffer.appendSlice("- **ORCID**: [");
                try buffer.appendSlice(orcid);
                try buffer.appendSlice("](https://orcid.org/");
                try buffer.appendSlice(orcid);
                try buffer.appendSlice(")\n");
            }
            ...
            for (author.affiliations) |aff| {
                ...
                if (aff.country) |country| {
                    try buffer.appendSlice(", ");
                    try buffer.appendSlice(country);
                }
                try buffer.appendSlice("\n");
            }

            if (author.affiliations.len > 0 and author.affiliations[0].email) |email| {
                try buffer.appendSlice("- **Email**: ");
                try buffer.appendSlice(email);
                try buffer.appendSlice("\n");
            }
        ...
        if (self.hasEqualContribution()) {
            try buffer.appendSlice("*† Equal contribution");
            if (self.equal_contribution_note) |note| {
                try buffer.appendSlice(" (");
                try buffer.appendSlice(note);
                try buffer.appendSlice(")");
...
test "AuthorList formatAsLaTeX" {
    const affils = [_]Affiliation{
        .{
            .institution = "Trinity Research Labs",
    ...
    const authors = [_]DetailedAuthor{
        .{
            .first_name = "John",
            .last_name = "Doe",
            .affiliations = &affils,
    ...
    const list = AuthorList{
        .title = "Authors",
        .authors = &authors,
        .corresponding_email = "john@trinity.ai",
    };

    const latex = try list.formatAsLaTeX(std.testing.allocator);
    defer std.testing.allocator.free(latex);

    try std.testing.expect(std.mem.indexOf(u8, latex, "\\author{") != null);
    try std.testing.expect(std.mem.indexOf(u8, latex, "John Doe") != null);
    try std.testing.expect(std.mem.indexOf(u8, latex, "Jane Smith") != null);
    try std.testing.expect(std.mem.indexOf(u8, latex, "Trinity Research Labs") != null);
}

test "AuthorList formatAsMarkdown" {
    const affils = [_]Affiliation{
        .{
    ...
    const md = try list.formatAsMarkdown(std.testing.allocator);
    defer std.testing.allocator.free(md);

    try std.testing.expect(std.mem.indexOf(u8, md, "# Author List") != null);
    try std.testing.expect(std.mem.indexOf(u8, md, "Alice Johnson") != null);
    try std.testing.expect(std.mem.indexOf(u8, md, "Test University") != null);
}
