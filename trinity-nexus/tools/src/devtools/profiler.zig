...
pub const Profiler = struct {
    ...
    pub fn init(allocator: Allocator, config: ProfilerConfig) Profiler {
        return .{
            ...
            .call_stack = ArrayList(CallFrame).empty,
    ...
    pub fn getHotFunctions(self: *const Profiler, limit: usize) ![]const u32 {
        var entries = ArrayList(struct { id: u32, time: u64 }).empty;
        defer entries.deinit(self.allocator);

        var iter = self.function_stats.iterator();
        while (iter.next()) |entry| {
            try entries.append(self.allocator, .{ .id = entry.key_ptr.*, .time = entry.value_ptr.total_time_ns });
        }
