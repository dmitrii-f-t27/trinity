...
pub const AgentStreamContext = struct {
    ...
    pub fn streamResponse(self: *Self, response: []const u8) !std.ArrayList([]u8) {
        ...
        while (i < response.len) {
            ...
            try events.append(allocator, @constCast(event)); // Cast because it expects []u8 but sendToken might return []const u8
        ...
        try events.append(allocator, @constCast(done_event));
