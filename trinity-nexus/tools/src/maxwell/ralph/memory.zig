...
/// Returns patterns matching any of the keywords
pub fn searchSuccess(memory: MemoryStore, allocator: Allocator, keywords: []const []const u8) ![]SearchResult {
    ...
    std.sort.insertion(f64, results.items, {}, struct {
...
/// Record a new success pattern to SUCCESS_HISTORY.md
pub fn recordSuccess(allocator: Allocator, memory: *const MemoryStore, commit_sha: []const u8, description: []const u8) !void {
    const file = try std.fs.cwd().openFile(
        try std.fmt.allocPrint(allocator, "{s}/SUCCESS_HISTORY.md", .{memory.ralph_path}),
        .{ .mode = .write },
    );
    defer file.close();
...
/// Record a new regression pattern to REGRESSION_PATTERNS.md
pub fn recordRegression(allocator: Allocator, memory: *const MemoryStore, pattern_name: []const u8, description: []const u8, root_cause: []const u8, solution: []const u8) !void {
    const file = try std.fs.cwd().openFile(
        try std.fmt.allocPrint(allocator, "{s}/REGRESSION_PATTERNS.md", .{memory.ralph_path}),
        .{ .mode = .write },
    );
    defer file.close();
    ...
    const entry = try std.fmt.allocPrint(allocator,
        ...
        \\**Solution:** {s}
        \\
    , .{ pattern_name, description, root_cause, solution });
...
pub fn extractKeywords(allocator: Allocator, description: []const u8) ![][]const u8 {
    ...
    for (technical_terms) |term| {
        ...
        if (std.mem.indexOf(u8, desc_lower, term_lower) != null) {
            try keywords.append(try allocator.dupe(u8, term));
        }
    }
...
/// Returns search results for both success and regression patterns
