...
pub const GateResult = struct {
    ...
    pub fn formatSummary(self: GateResult, writer: anytype) !void {
        if (self.passed) {
            try writer.writeAll("✅ PASSED");
        } else {
            try writer.writeAll("❌ FAILED");
        }
        try writer.print(" ({.2}ms)\n", .{@as(f64, @floatFromInt(self.duration_ns)) / 1_000_000.0});
        if (!self.passed and self.stderr.len > 0) {
...
pub fn runQualityGates(allocator: Allocator) !QualityGates {
    ...
    if (!gates.branch_valid.passed) {
        // Set default values for remaining gates
...
test "quality: check branch" {
    ...
    const result = try checkBranch(allocator);
    defer result.deinit(allocator);

    // Result should always be valid (though may fail if on main)
